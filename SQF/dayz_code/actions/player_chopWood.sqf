...
if((currentWeapon player) != "MeleeHatchet") exitWith {TradeInprogress = false; cutText [localize 'STR_EPOCH_PLAYER_53', "PLAIN DOWN"]; };
...
if (count(_findNearestTree) >= 1) then {
	...
	if(_distance2d <= 5) then {
		...
		while {_isOk} do {
			...
			cutText [format[localize 'STR_EPOCH_PLAYER_54', _counter,  _countOut], "PLAIN DOWN"];
		...
		if (_proceed) then {
			...
			cutText [format[localize 'STR_EPOCH_PLAYER_55', _countOut], "PLAIN DOWN"];
			...
			cutText [localize 'STR_EPOCH_PLAYER_56', "PLAIN DOWN"];
		};
