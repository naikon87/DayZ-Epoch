...
if(TradeInprogress) exitWith { cutText [localize 'STR_EPOCH_PLAYER_113' , "PLAIN DOWN"] };
	...
	if(_x in _availableCansEmpty) then {
		...
		if(_canSizeEmpty == 0) then {
			...
			if (_newFuel > 0) then {
				...
				cutText [format[localize 'STR_EPOCH_PLAYER_31',_canTypeEmpty], "PLAIN DOWN"];
				...
				if (_finished) then {
					...
					if (_newFuel > 0) then {
						...
						if(([player,_canNameEmpty] call BIS_fnc_invRemove) == 1) then {
							...
							cutText [format[localize 'STR_EPOCH_PLAYER_114',_nameText,_canSize], "PLAIN DOWN"];
						...
						cutText [format[localize 'STR_EPOCH_PLAYER_115',_nameText], "PLAIN DOWN"];
						_abort = true;
					};
						
				} else {
					cutText [localize 'STR_EPOCH_PLAYER_32' , "PLAIN DOWN"];
					_abort = true;
				};
