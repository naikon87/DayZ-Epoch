...
if(TradeInprogress) exitWith { cutText [localize 'STR_EPOCH_PLAYER_156' , "PLAIN DOWN"]; };
TradeInprogress = true;

// Test cannot lock while another player is nearby
_playerNear = {isPlayer _x} count (player nearEntities ["CAManBase", 6]) > 1;
if(_playerNear) exitWith { TradeInprogress = false; cutText [localize 'STR_EPOCH_PLAYER_157' , "PLAIN DOWN"];  };
...
if (_alreadyPacking == 1) exitWith {TradeInprogress = false; cutText [format[localize 'STR_EPOCH_PLAYER_158',_text], "PLAIN DOWN"]};
...
if ((_ownerID == dayz_combination) or (_ownerID == dayz_playerUID)) then {
	...
	if (_claimedBy == _playerID) then {
		...
		if(!isNull _obj and alive _obj) then {
			...
			cutText [format[localize 'STR_EPOCH_PLAYER_159',_text], "PLAIN DOWN"];
		};
	} else {
	...
	sleep 5;
	cutText [format[localize 'STR_EPOCH_PLAYER_160',_text], "PLAIN DOWN"];
};
s_player_unlockvault = -1;
TradeInprogress = false;